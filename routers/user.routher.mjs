...
import { UserFollowModel } from "../models/users-follow.model.mjs";
...
router.get("/:username", async (req, res) => {
  ...
  if (!user) {
    return res
      .status(404)
      .send({ message: `User with ${username} not found!` });
  }
...
router.post("/:username/follow", authMiddleware, async (req, res) => {
  ...
  if (!existingFollow) {
    ...
    return res
      .status(200)
      .send({ message: "Амжилттай фоллов хийлээ", isfollowed: true });
  }

  await UserFollowModel.findOneAndDelete(existingFollow._id);
  return res
    .status(200)
    .send({ message: "Амжилттай фоллов буцаалаа", isfollowed: false });
});
